...
class Notifier {
  ...
  formatUserAction(userInfo, action, additionalData = {}) {
    ...
    message += `🖥️ <b>IP/Регион:</b> ${userInfo.estimatedCountry} (по языку)\n`; // Заменили тип чата на IP/Регион
    ...
    message += `🗣️ <b>Язык:</b> ${userInfo.languageCode || 'не указан'}\n`;
  ...
  async sendBotStats() {
    ...
    try {
      ...
      // Получаем распределение по странам
      const countries = await UserSession.aggregate([
...
🌍 <b>Топ стран по пользователям:</b>
`;

      countries.forEach((country, index) => {
        statsMessage += `${index + 1}. ${country._id}: ${country.count} пользователей\n`;
      });
